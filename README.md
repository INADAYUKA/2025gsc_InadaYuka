...
## 結果/Result
衛星データやハザードマップを、Blenderで作成した3D地図と重ねることによる分析を行った。まず、Copernicus BrowserではSentinel-1のUrban Areas Scriptを用い、イベント開催日や時期的要因による都市の賑わいの違いを比較した。衛星データは人流を直接示すものではないが、異なる日の都市へのレーダー反射の差異を比較することで、人々による活動の増減を間接的に捉えることを目的とした。比較対象として、ハロウィンやクリスマス、新学期開始日、大規模イベントの開催日など複数の日付を選定した。その結果、複数のイベントが重なった日には、他の日と比べてより広範囲で高い反射が確認され、想像以上に賑わいが集中している様子が見られた。
